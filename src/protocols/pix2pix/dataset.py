...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(img_path)
        if idx in self.cached_data and self.config.dataset.cache:
        ...
        if self.df.loc[idx, 'Dataset'] == 'JSRT':
            radius = int(self.df.loc[idx, 'Size']/2)
            y_min_wide, y_max_wide = y_min - radius, y_max + radius
            x_min_wide, x_max_wide = x_min - radius, x_max + radius
            if y_min_wide > 0 and x_min_wide > 0 and y_max_wide < max(target.shape) and x_max_wide < max(target.shape):
                target[y_min_wide:y_max_wide, x_min_wide:x_max_wide, :] = 0
            else:
    ...
    def show(image=None, target=None, output=None, landmarks=None, plot_point_width=3, min_px_value=0, max_px_value=1):
        ...
        if len(empty_shape) == 4:
            ...
            ax[2].imshow(np.zeros(empty_shape)) if output is None else ax[2].imshow(output,
                                                                                    vmin=min_px_value,
                                                                                    vmax=max_px_value)
            # TODO: find out why output is plotted not in grayscale
            if landmarks is not None:
...
def save_inference(config, phase, input_d, output, tag='default_tag'):
    ...
    min_px_value, max_px_value = utils.PIXEL_RANGES[config.model.G.final_activation[0]['name']]
