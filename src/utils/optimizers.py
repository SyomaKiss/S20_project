...
def get_optimizer(parameters, entry):
    """
    Returns optimizer either from torch or implemented in 'optimizers' module.
    :param parameters: iterable of parameters to optimize or dicts defining parameter groups
    :param entry: experiment configuration module for optimizer
    :return: optimizer
    """
    entry = entry[0]
    kwargs = {key: entry[key] for key in entry.keys() if key not in ["name"]}
    ret = globals()[entry["name"]](parameters, **kwargs)
    return ret
